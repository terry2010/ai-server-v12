...
- 首页 Dashboard 将 **AI 浏览器** 视为一个独立模块卡片，整体交互风格参考 n8n/OneAPI 等模块：
  - 卡片显示内容包括：模块名称（AI 浏览器）、简要说明、当前启用状态等；
  - 点击卡片的「打开」按钮后，路由跳转到 `/browser-agent` 页面，在顶部标签栏中与其它模块共存；
  - 顶部标签栏仍沿用现有实现，只是 `/browser-agent` 页面本身不创建 Docker 模块对应的 BrowserView，而是展示 AI 浏览器任务 UI；
- **显示条件**：仅当 `settings.browserAgent.enabled === true` 时，在首页模块列表中渲染 AI 浏览器模块卡片；
  - 当 Browser Agent 被在「系统设置 → Agent 设置」中关闭时，Dashboard 不显示该卡片；
  - 这样可以避免在未启用 Browser Agent 的环境中出现“死入口”。
- **侧边导航栏**：左侧 SideNav 不增加「AI 浏览器」或类似入口，只保留 Dashboard 模块卡片这一种进入方式；
  - 侧边栏仍仅包含：仪表盘 / 在线教程 / AI 市场 / 系统设置 / 系统日志 / 性能监控 等全局功能。
